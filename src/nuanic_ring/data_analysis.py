...
def load_nuanic_csv(filepath: str) -> pd.DataFrame:
    ...
    df['stress_raw'] = pd.to_numeric(df.get('stress_raw'), errors='coerce').fillna(0).astype(int)
    df['stress_percent'] = pd.to_numeric(df.get('stress_percent'), errors='coerce').fillna(0.0)
...
def fit_mm_equation_from_export(df: pd.DataFrame) -> dict[str, Any] | None:
    ...
    ret = {
        ...
        "rows_available_3f": len(valid_df.dropna(subset=['eda'])) if 'eda' in valid_df.columns else 0,
        "beta": {
...
def print_export_fit_report(filepath: str) -> bool:
    ...
    print(f"Valid values -> dne: {valid_dne}, srl: {valid_srl}, srrn: {valid_srrn}, eda: {valid_eda}")
    ...
    print(f"Rows available (2-feature): {fit['rows_available_2f']} | (3-feature): {fit['rows_available_3f']}")
    
    b = fit["beta"]
    print("\nFitted model (standardized features):")
    if fit["mode"] == "scrn+scl_us+eda":
        print(f"dne ~= {b['intercept']:.4f} + {b['scrn_z']:.4f}*z(scrn) + {b['scl_us_z']:.4f}*z(scl_us) + {b['eda_z']:.4f}*z(eda)")
    else:
        print(f"dne ~= {b['intercept']:.4f} + {b['scrn_z']:.4f}*z(scrn) + {b['scl_us_z']:.4f}*z(scl_us)")
...
def detect_peaks(series: pd.Series | list, threshold_std: float = 1.5) -> list[dict[str, float | int]]:
...
def calculate_correlation(s1: pd.Series | list[float], s2: pd.Series | list[float]) -> float:
    """Vectorized Pearson correlation leveraging pure C."""
    if isinstance(s1, list): s1 = pd.Series(s1)
    if isinstance(s2, list): s2 = pd.Series(s2)
    ...
    if eda_channels:
        ...
        for channel in eda_channels:
            ...
            if len(channel["values"]) == len(df):
                correlation = calculate_correlation(channel["values"], df["stress_percent"])
